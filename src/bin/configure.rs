    ...
    -> Result<(), Box<dyn std::error::Error>>
{
    ...
    //0x00FD
    println!("Read  0x0E register  ");
    let block_resp: Vec<u16> = ctx.read_holding_registers(0x0E, 2).await??;
    println!("> block_resp 0x0E: {:?}", block_resp);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //     println!(" IV ADC ch2_value: {ch2_value:?} = {verified_milliamps:?} mA");
        ...
        println!("Check NODEID_N4AIA04_ADC...");
        ctx.set_slave(Slave(NODEID_N4AIA04_ADC));
        let read_rsp: Vec<u16> = ctx.read_holding_registers(0x00, 16).await??;
        println!(" N4AIA04  (0x00)[4]: {read_rsp:?}");
