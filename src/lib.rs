...
//===== Cycloid-based periodic potential generation =====
...
pub struct CycloidLut {
    /// Voltage samples at uniform time intervals
    samples: Vec<f64>,
...
impl CycloidLut {
    ...
    /// O(1) lookup with linear interpolation between two adjacent uniform samples.
    pub fn voltage_at_ms(&self, timestamp_ms: i64) -> f64 {
        let query_ms = (timestamp_ms as f64).rem_euclid(self.period_ms);

        let pos = query_ms / self.bucket_ms;  // exact position in sample space
        let lo_idx = pos.floor() as usize;
        let hi_idx = (lo_idx + 1).min(self.samples.len() - 1);

        let t = pos.fract(); // fractional part = interpolation weight
        self.samples[lo_idx] + t * (self.samples[hi_idx] - self.samples[lo_idx])
    }

    /// Precalculate periodic Cycloid LUT from parameters
    pub fn create_with(period_secs: u32, peak_val: f32, floor_val: f32, nsteps: u32) -> Self {
        // precalculate lookup table of value vs time for cycloid potential
        let raw_table = Self::generate_nonuniform_cycloid_table(
            period_secs as f64, peak_val as f64, floor_val as f64, nsteps as usize);
        Self::from_nonuniform_table(&raw_table, nsteps as usize)
    }

    /// Used only during LUT construction, to map nonuniform table to uniform (time) grid table
    fn interpolate_from_nonuniform_table(table: &[CycloidPoint], query_ms: f64) -> f64 {
        if query_ms <= table.first().unwrap().t_ms {
            return table.first().unwrap().voltage;
        }
        if query_ms >= table.last().unwrap().t_ms {
            return table.last().unwrap().voltage;
        }
        let idx = table.partition_point(|p| p.t_ms <= query_ms);
        let lo = &table[idx - 1];
        let hi = &table[idx];
        let span = hi.t_ms - lo.t_ms;
        if span == 0.0 { return lo.voltage; }
        let t = (query_ms - lo.t_ms) / span;
        lo.voltage + t * (hi.voltage - lo.voltage)
    }

    /// Generate a nonuniform (in time) cycloid table based on theta parametric
    fn generate_nonuniform_cycloid_table(period_secs: f64, peak_v: f64, floor_v: f64, steps: usize) -> Vec<CycloidPoint> {
        let mut table = Vec::with_capacity(steps);
        let r = peak_v / 2.0;
        for i in 0..=steps {
            let theta = (i as f64 / steps as f64) * 2.0 * std::f64::consts::PI;
            let t_secs = (period_secs / (2.0 * std::f64::consts::PI)) * (theta - theta.sin());
            let mut v = r * (1.0 - theta.cos());
            if v < floor_v { v = floor_v; }
            table.push(CycloidPoint {
                t_ms: t_secs * 1000.0,
                voltage: v,
            });
        }
    ...
    /// Build ourselves from the raw (non-uniform) cycloid table by resampling onto a uniform grid.
    fn from_nonuniform_table(table: &[CycloidPoint], num_buckets: usize) -> Self {
        let period_ms = table.last().unwrap().t_ms;
        let bucket_ms = period_ms / num_buckets as f64;

        let mut samples = Vec::with_capacity(num_buckets + 1);

        for i in 0..=num_buckets {
            let query_ms = i as f64 * bucket_ms;
            // One-time binary search during construction — free at runtime
            let v = Self::interpolate_from_nonuniform_table(table, query_ms);
            samples.push(v);
        }

        CycloidLut { samples, bucket_ms, period_ms }
    }
